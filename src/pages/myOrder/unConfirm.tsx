...
export default function UnConfirm() {
  ...
  const getDataProduct = async () => {
    try {
      const currentPage = 1;
      setPage(currentPage);
      const res = await myOderApi.myOder(0, currentPage); //chưa xác nhận
      if (res.status) {
  ...
  const handleCancel = async (id: number) => {
    try {
      const res = await myOderApi.cancelOrder(id);
      if (res.data.code === 200) {
        setDescription("Huỷ đơn hàng thành công");
        setShowAlert(true);
        getDataProduct();
      } else {
        setDescription("Huỷ đơn hàng thất bại.Thử lại sau");
        setShowAlert(true);
      }
    } catch (error) {
      console.log(error);
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  className="bg-blue rounded-xl w-fit ml-2 my-2 py-1 px-4 flex items-center justify-center"
                >
